...
class ReportingEngine:
    ...
    async def _generate_balance_reports(self) -> Dict[str, Any]:
        """Generate balance reports using lightweight cached data instead of fresh API calls"""
        result = {"generated": False, "sent": False}
        ...
        try:
            ...
            # Use lightweight DB-based balance state instead of calling monitor_all_balances()
            # which makes expensive fresh API calls to Fincra/Kraken/FastForex
            if Config.ADMIN_EMAIL:
                timestamp = datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
                
                # Read last known balance state from DB (set by reconciliation job)
                balance_summary = "Balance data from last reconciliation cycle"
                try:
                    from database import sync_managed_session
                    from sqlalchemy import text
                    with sync_managed_session() as session:
                        result_rows = session.execute(
                            text("SELECT alert_key, provider, currency, alert_level, last_alert_time FROM balance_alert_state ORDER BY updated_at DESC LIMIT 10")
                        )
                        rows = result_rows.fetchall()
                        if rows:
                            balance_lines = []
                            for row in rows:
                                balance_lines.append(f"  • {row[1]} ({row[2]}): Level={row[3] or 'OK'}, Last Alert={row[4] or 'N/A'}")
                            balance_summary = "\n".join(balance_lines)
                except Exception as db_err:
                    logger.debug(f"Could not read balance state from DB: {db_err}")
                    balance_summary = "Balance state unavailable - check reconciliation logs"
                
                subject = f"📊 {Config.PLATFORM_NAME} Daily Balance Report - {datetime.utcnow().strftime('%B %d, %Y')}"
                email_content = f"""
📊 DAILY BALANCE REPORT (Lightweight)
{Config.PLATFORM_NAME} Platform - {timestamp}

BALANCE ALERT STATE:
{balance_summary}

Note: Full balance checks run every 30 minutes via reconciliation.
This report uses cached state to avoid unnecessary API calls.
