    ...
    def _credit_wallet_immediate(
        ...
        try:
            ...
            transaction = Transaction(
                user_id=user_id,
                transaction_type="wallet_deposit",  # Consistent with existing records
                amount=usd_amount,
                currency="USD",
                status="completed",
                transaction_id=f"DEPOSIT-{provider.upper()}-{txid}-{user_id}",  # Strengthened uniqueness
                description=f"Payment: {amount} {currency.upper()} from {provider}",
                tx_hash=txid,  # Store the blockchain transaction hash
                from_address=raw_data.get('address_in', ''),
                to_address=raw_data.get('address_out', ''),
                blockchain_address=raw_data.get('address_in', ''),  # For auditability
                currency_pair=f"{currency.upper()}_USD"
            )
