-- Database Migration: Add Unified Transaction Schema
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_transaction_id 
    ON unified_transactions(transaction_id);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_user_id 
    ON unified_transactions(user_id);

-- Core business logic indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_user_type_status 
    ON unified_transactions(user_id, transaction_type, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_status_created 
    ON unified_transactions(status, created_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_type_status 
    ON unified_transactions(transaction_type, status);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_amount_currency 
    ON unified_transactions(amount, currency);

-- External reference indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_external_ref 
    ON unified_transactions(external_reference_id) WHERE external_reference_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_blockchain_hash 
    ON unified_transactions(blockchain_tx_hash) WHERE blockchain_tx_hash IS NOT NULL;

-- Retry system indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_retry_schedule 
    ON unified_transactions(next_retry_at, status) WHERE next_retry_at IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_error_code 
    ON unified_transactions(last_error_code) WHERE last_error_code IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_failure_type 
    ON unified_transactions(failure_type, status) WHERE failure_type IS NOT NULL;

-- Timeout and expiry indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_payment_timeout 
    ON unified_transactions(payment_timeout_at, status) WHERE payment_timeout_at IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_processing_timeout 
    ON unified_transactions(processing_timeout_at, status) WHERE processing_timeout_at IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_auto_expire 
    ON unified_transactions(auto_expire_at, status) WHERE auto_expire_at IS NOT NULL;

-- Related entity indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_escrow 
    ON unified_transactions(escrow_id) WHERE escrow_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_cashout 
    ON unified_transactions(cashout_id) WHERE cashout_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_exchange 
    ON unified_transactions(exchange_order_id) WHERE exchange_order_id IS NOT NULL;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_parent 
    ON unified_transactions(parent_transaction_id) WHERE parent_transaction_id IS NOT NULL;

-- Status History indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_history_transaction 
    ON unified_transaction_status_history(transaction_id, changed_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_history_status 
    ON unified_transaction_status_history(to_status, changed_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_history_trigger 
    ON unified_transaction_status_history(triggered_by, changed_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_history_user 
    ON unified_transaction_status_history(triggered_by_user_id) WHERE triggered_by_user_id IS NOT NULL;

-- Retry Logs indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_retry_transaction 
    ON unified_transaction_retry_logs(transaction_id, attempted_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_retry_error_code 
    ON unified_transaction_retry_logs(error_code, attempted_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_retry_result 
    ON unified_transaction_retry_logs(retry_result, attempted_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_retry_schedule 
    ON unified_transaction_retry_logs(next_scheduled_retry) WHERE next_scheduled_retry IS NOT NULL;

-- Metadata indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_metadata_transaction 
    ON unified_transaction_metadata(transaction_id, metadata_category);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_metadata_key 
    ON unified_transaction_metadata(metadata_key, metadata_category);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_metadata_access 
    ON unified_transaction_metadata(access_level, is_sensitive);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_unified_tx_metadata_expires 
    ON unified_transaction_metadata(expires_at) WHERE expires_at IS NOT NULL;
...
SELECT 
    ...
    tableowner
FROM pg_tables 
WHERE tablename LIKE 'unified_transaction%'
ORDER BY tablename;
...
WHERE tc.table_name LIKE 'unified_transaction%'
