                ...
                try:
                    ...
                    if existing_escrow:
                        ...
                        if existing_escrow.payment_confirmed_at is not None:
                            ...
                            total_fee = Decimal(str(getattr(existing_escrow, 'fee_amount', 0) or 0))
                            seller_fee = Decimal(str(getattr(existing_escrow, 'seller_fee_amount', 0) or 0))
                            escrow_amount = Decimal(str(existing_escrow.amount or 0))
                            
                            if fee_split_option == "seller_pays" and total_fee > 0:
                                # Seller was supposed to pay: deduct full fee from escrow refund
                                refund_amount = escrow_amount - total_fee
                                logger.info(f"💰 SELLER_PAYS_CANCEL: Buyer absorbs full fee ${total_fee}, refund=${refund_amount}")
                            elif fee_split_option == "split" and seller_fee > 0:
                                # Split: buyer already paid buyer_fee, now absorbs seller's portion too
                                refund_amount = escrow_amount - seller_fee
                                logger.info(f"💰 SPLIT_FEE_CANCEL: Buyer absorbs seller fee ${seller_fee}, refund=${refund_amount}")
                            else:
                                # buyer_pays: refund trade amount only (fee already retained from frozen)
    ...
    try:
        with atomic_transaction() as session:
            ...
            if original_status == "payment_confirmed":
                ...
                # BUSINESS RULE: On buyer cancellation, buyer ALWAYS pays FULL platform fee
                # regardless of original fee split option (buyer_pays, seller_pays, or split)
                if fee_split_option == "seller_pays":
                    # Seller was supposed to pay: deduct full fee from escrow refund
                    amount_decimal = trade_amount_decimal - total_fee_decimal
                    logger.info(f"💰 SELLER_PAYS_CANCEL: Buyer absorbs full fee ${total_fee_decimal}, refund=${amount_decimal}")
                elif fee_split_option == "split" and seller_fee_decimal > 0:
                    # Split: buyer already paid buyer_fee, now absorbs seller's portion too
